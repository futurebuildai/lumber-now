package anthropic

import (
	"context"

	"github.com/builderwire/lumber-now/backend/internal/domain"
	"github.com/builderwire/lumber-now/backend/internal/store/db"
)

const pdfSystemPrompt = `You are an AI assistant for a lumber and building materials dealer. Your job is to analyze PDF documents (takeoff sheets, material lists, invoices, specs) and extract structured material requests.

For each item identified, extract:
- sku: Match to the closest SKU from the inventory catalog if possible, otherwise leave empty
- name: The item name/description
- quantity: Numeric quantity
- unit: Unit of measure (EA, BF, LF, SF, etc.)
- confidence: Your confidence in the extraction from 0.0 to 1.0
- matched: Whether you found a matching SKU in the catalog
- notes: Any relevant notes about the source document context

Respond with ONLY a JSON array of items. No other text.`

func (c *Client) ParsePDF(ctx context.Context, pdfURL string, inventory []db.Inventory) ([]domain.StructuredItem, float64, error) {
	invContext := buildInventoryContext(inventory)

	msgs := []message{
		{
			Role: "user",
			Content: []content{
				{
					Type: "document",
					Source: &source{
						Type: "url",
						URL:  pdfURL,
					},
				},
				{Type: "text", Text: invContext + "\n\nPlease analyze this document and extract all material items."},
			},
		},
	}

	resp, err := c.call(ctx, "claude-sonnet-4-5-20250929", pdfSystemPrompt, msgs)
	if err != nil {
		return nil, 0, err
	}

	return parseItemsFromResponse(resp)
}
